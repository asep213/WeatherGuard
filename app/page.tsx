      ...
      <main className="main">
        ...
        {mode === "map" ? <MapPanel locations={locations} onPick={pickMapPoint} /> : <>
          ...
          <section className="content-grid">
            ...
            <div className="panel"><div className="panel-head"><div><h2>Peta risiko wilayah</h2><span className="panel-subtitle">Klik peta untuk titik mana pun di Indonesia</span></div><Map size={18} color="#087e8b" /></div><MapPanel locations={[activeLocation]} onPick={pickMapPoint} compact /></div>
          </section>
        </>}
      </main>
    </div>
...
function MapPanel({ locations, compact = false, onPick }: { locations: Location[]; compact?: boolean; onPick?: (lat: number, lon: number) => void }) { return <div className={`panel ${compact ? "map-panel" : "wide"}`}><div className="panel-head"><div><h2>{compact ? "" : "Peta interaktif"}</h2><span className="panel-subtitle">{compact ? "" : "OpenStreetMap • klik lokasi untuk melihat data cuaca"}</span></div></div><div className="map"><WeatherMap locations={locations} onPick={onPick} /><span className="map-label">Klik peta untuk forecast real • Open-Meteo</span></div></div>; }